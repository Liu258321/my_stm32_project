...
u8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 2; 
        printf("set configure fifo success\n");
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样率
		if(res)return 3; 
        printf("set sample rate success\n");
		res=dmp_load_motion_driver_firmware();		//加载dmp固件
		if(res)return 4; 
        printf("load motion driver firware success\n");
...
//返回值:0,正常
//    其他,失败
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw,float *x_acc,float *y_acc,float *z_acc)
{
